...
int main()
{
	...
	std::cout << "Use base85 encode?";
	std::cin >> use_base85_encoding;

	std::cout << "\nUse compression?";
	std::cin >> use_compression;
...
GET_FONTNAME:
	...
	arguments[1].erase(arguments[1].find_first_of("."), arguments[1].size());
	arguments[1] = "C_array_" + arguments[1];

	if (!binary_to_compressed_c(arguments[0].c_str(), arguments[1].c_str(), use_base85_encoding, use_compression))
...
bool binary_to_compressed_c(const char* filename, const char* symbol, bool use_base85_encoding, bool use_compression)
{
	...
	std::ofstream o(symbol + std::string(".h"));
	FILE* out = fopen((symbol + std::string(".h")).c_str(), "w");
	...
	{
		fprintf(out, "static const char %sfont_data_base85[%d+1] =\n    \"", compressed_str, (int)((compressed_sz + 3) / 4) * 5);
		char prev_c = 0;
	...
	{
		fprintf(out, "static const unsigned int %sfont_data[%d] =\n{", compressed_str, (int)((compressed_sz + 3) / 4));
		int column = 0;
		for (int i = 0; i < compressed_sz; i += 4)
		{
			unsigned int d = *(unsigned int*)(compressed + i);
			if (compressed_sz - 1 == i)
			{
				if ((column++ % 12) == 0)
					fprintf(out, "\n    0x%08x", d);
				else
